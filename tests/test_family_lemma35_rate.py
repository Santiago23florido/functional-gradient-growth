...
class _ForceScalarWeight:
    """Deterministic optimizer that makes the family clone overshoot."""
...
def _transaction_problem():
    model = torch.nn.Linear(1, 1, bias=False)
    with torch.no_grad():
        model.weight.fill_(1.0)
    return model, torch.ones((1, 1)), torch.zeros((1, 1))


def test_family_transaction_backtracks_from_the_same_trained_clone(
    monkeypatch,
) -> None:
    """The full clone does not descend; a smaller endpoint does and commits."""
    monkeypatch.setattr(torch.optim, "AdamW", _ForceScalarWeight)
    config = _config(
        rel_error_threshold=0.5,
        certify_family_lemma35_rate=True,
    ...
    model, x, y = _transaction_problem()
    ...
    result = certify_parametric_step(
        ...
        y,
        config,
        functional_learning_rate=2.0,
        inner_steps=1,
    )

    assert result.certified is True
    assert result.transactional_trial_count == 2
    assert result.committed_parameter_fraction == pytest.approx(0.25)
    assert result.functional_before == pytest.approx(1.0)
    assert result.functional_after == pytest.approx(0.0)
    assert result.predicted_functional_decrease == pytest.approx(1.0)
    assert result.transactional_rejected is False
    assert torch.equal(model.weight, original), "the base model is never a trial"
    assert result.model is not None
    assert result.model.weight.item() == pytest.approx(0.0)


def test_family_transaction_returns_no_model_when_every_endpoint_fails(
    monkeypatch,
) -> None:
    """A rejected family cannot leak its last attempted endpoint to the loop."""
    monkeypatch.setattr(torch.optim, "AdamW", _ForceScalarWeight)
    config = _config(
        rel_error_threshold=0.5,
        transactional_realized_descent=True,
        transactional_max_retries=1,
        transactional_backtrack_factor=0.5,
    )
    model, x, y = _transaction_problem()
    original = model.weight.detach().clone()
    ...
    assert result.model is None
    assert result.transactional_trial_count == 2
    assert result.committed_parameter_fraction is None
    assert result.transactional_rejected is True
